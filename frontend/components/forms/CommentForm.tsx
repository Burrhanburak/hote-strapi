...
import { makeComment } from "@/lib/action/commentActions";
...
interface CommentFormProps {
  userId: number;
  roomId: number;
  onCommentAdded?: () => void; // Callback to refresh the comment list
}
...
const CommentForm: React.FC<CommentFormProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    if (!token || !userResponse.ok || !userResponse.data) {
      toast.error("You need to sign in to submit a comment.");
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      // Ensure the rating is within the expected range, e.g., 1 to 5
      ...
      const response = await makeComment(roomId, content, rating, userId);
      if (response.data.error) {
        ...
        if (onCommentAdded) onCommentAdded(); // Optionally refresh the comment list
      }
    } catch (error) {
      toast.error("Failed to add comment.");
